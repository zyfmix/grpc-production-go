...
import (
	...
	rpcError "grpcs/src/rpc/error"
)

var (
	// 自定义业务错误码
	ErrorLoginDail = NewError(20001, "登录失败")
...
func ToGRpcStatus(err *Error, msg string) *Status {
	s, _ := status.New(ToRpcCode(err.code), msg).WithDetails(&rpcError.Error{Code: int32(err.code), Message: err.msg})
	return &Status{s}
}

// ToGRpcError 通过Details属性返回错误信息给外部客户端
func ToGRpcError(err *Error) error {
	s, _ := status.New(ToRpcCode(err.code), err.msg).WithDetails(&rpcError.Error{Code: int32(err.code), Message: err.msg})
	return s.Err()
}
